...
const Calendar = React.createClass({
  ...
  onSelect(value) {
    this.props.onWorkoutSelect(value.format('YYYY-MM-DD'), false);
  ...
  getMiddleValue() {
    const middleValue = this.state.value.clone();
    middleValue.add(1, 'months');
    const selectedValue = this.state.selectedValue;
    // keep selectedTime when select date
    if (selectedValue[1] && this.props.timePicker) {
      syncTime(selectedValue[1], middleValue);
    }
  ...
  render() {
    ...
    const getMiddleValue = this.getMiddleValue();
