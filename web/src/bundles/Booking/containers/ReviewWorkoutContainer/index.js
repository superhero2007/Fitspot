...
import { estimatePrice } from '@store/modules/booking/actions'
...
class ReviewWorkoutContainer extends Component {
  ...
  componentWillReceiveProps(nextProps) {
    ...
    } else if(!nextProps.error && nextProps.customerPrice) {
  ...
  processPromo(promocode) {
    ...
    this.props.estimatePrice(bookingState.chosenLocation.lat,
        ...
        bookingState.numFriends, promocode)
  }
...
const mapDispatchToProps = (dispatch) => {
  return {
    ...
    estimatePrice: (lat, lon, radius, gymId, dateTime, numFriends, promoCode) => {
      dispatch(estimatePrice(lat, lon, radius, gymId, dateTime, numFriends, promoCode))
    },
  }
}
