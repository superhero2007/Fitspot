...
class EditCustomerProfile extends React.Component{
  ...
  renderStatLine(number,shortCode,title,property,value,maxLength){
    return(
      <View>
        <TextInput style={{fontFamily:FONT_DAYTONA_REG,color:'#5D5D5F',fontSize:24, width: 50, height: 30,textAlign:'center'}} placeholder={number} value={value.toString()}
          onChangeText={(text) => this.updateText(property,text)} maxLength={maxLength}
          />
  ...
  render (){
    ...
    return (
      ...
      <KeyboardAwareScrollView style={{marginBottom:55}} contentContainerStyle={{marginTop:63,marginBottom: 55}}>
        <Button buttonStyle={[styles.facebookButton,{marginTop:15,marginBottom:15}]} buttonTextStyle={styles.facebookButtonTextStyle} onPress={() => this.updatePhoto()}>
          Connect Facebook
        </Button>
        ...
        <HorizontalLine />
          <View style={{flexDirection:'row',justifyContent: 'space-around',marginTop:15,marginBottom: 15}}>
            ...
            />
              ...
              {this.renderStatLine('6',"m","Height (feet)","height", user.customer.height == null ? 0 : user.customer.height,3)}
              {this.renderStatLine('110',"kg","Weight (lbs)","weight", user.customer.weight == null ? 0 : user.customer.weight,3)}
          </View>
        <HorizontalLine />
        ...
        <Button buttonStyle={[styles.buttonStyle]} buttonTextStyle={styles.buttonTextStyle} onPress={Actions.bodyFocus}>
          Change Body Focus
        </Button>
        ...
        <Button buttonStyle={[styles.buttonStyle]} buttonTextStyle={styles.buttonTextStyle} onPress={Actions.trainerStyles}>
          Change Training Style
        </Button>
        <Button buttonStyle={[styles.buttonStyle]} buttonTextStyle={styles.buttonTextStyle} onPress={Actions.favHighIntensity}>
          Change High Intensity Workouts
        </Button>
        <Button buttonStyle={[styles.buttonStyle]} buttonTextStyle={styles.buttonTextStyle} onPress={Actions.favLowIntensity}>
          Change Low Intensity Workouts
        </Button>

        <View style={{marginTop : 20}}>
          <HorizontalLine />
          <TextInput style={styles.textInput} placeholder='Medical Issues (40 characters max)' maxLength={40}/>
        </View>
        <HorizontalLine/>
