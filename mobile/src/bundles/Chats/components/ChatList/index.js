...
class ChatList extends React.Component {
  ...
  constructor(props) {
    ...
    mMsgNotificationSize = getCorrectFontSizeForScreen(9);
  ...
  renderRow(rowData) {
    ...
    var numberOfUnread = this.state.user.userType === CONSTS.USER_TYPE.CUSTOMER ? rowData.userUnreadMessagesNumber : rowData.trainerUnreadMessagesNumber
    var boldText = numberOfUnread > 0
    ...
    return (
      ...
      <View style={{flex: 1,flexDirection: 'row', paddingTop:15,paddingBottom:15, borderBottomWidth:1,borderBottomColor: '#c0c0c0'}}>
        <View style={{marginLeft:12, marginRight:12}}>
          { userToDisplay.avatar === null ?
            <Image style={[{
              ...
              height: 50,
              // position:'absolute'
            }, numberOfUnread > 0 ? {borderColor:'#E67650',borderWidth:2} : {}]} source={require('@images/default-avatar.png')} />
          ...
          <Image style={[{
            ...
            height: 50,
            // position:'absolute'
          }, numberOfUnread > 0 ? {borderColor:'#E67650',borderWidth:2} : {}]} source={{uri: userToDisplay.avatar.url}} />
          }
        { numberOfUnread > 0 ?
        <View style={{backgroundColor:'#E67650',width:22,height:22,right:-5,top:-5,borderRadius:11,borderWidth:2,borderColor:'#E67650',position:'absolute'}}>
          <Text style={{textAlign:'center',justifyContent:'center',color:'white',fontSize:mMsgNotificationSize,fontFamily:FONT_DAYTONA_SEMIBOLD,width:25,marginLeft:-2.5,height:16,marginTop:2,backgroundColor:'rgba(0,0,0,0)'}}>{numberOfUnread}</Text>
        </View>
        :
        null
        }
        ...
        <View>
          ...
          <View style={{flex:1,flexDirection:'column'}}>
            <Text style={{flex:1, maxWidth:220,fontSize:mMsgNotificationSize,fontFamily:'System', fontWeight: boldText ? 'bold' : 'normal',color: '#7D7D80'}}>{rowData.lastMessage === null ? 'No messages sent.' : rowData.lastMessage.text }</Text>
          </View>
        </View>
      </View>
    </TouchableHighlight>
