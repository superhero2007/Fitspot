...
import {checkAuth, checkLocationPermission, sendDeviceToken, setAppAvailable, updatePaymentTitle} from '@store/modules/auth/actions'
...
class SplashContainer extends Component {
  ...
  handleLoggedIn(user, userProfileReady, braintreeToken) {
    ...
    if (braintreeToken != null) {
      BrainTree.getPaymentInformation(this.props.braintreeToken).then(value => {
        this.props.updatePayTitle(value);
      });
  ...
  componentWillReceiveProps(nextProps) {
    ...
    if (!nextProps.isFetching) {
      ...
      if (nextProps.userProfileReady && !this.state.userProfileReady) {
        ...
        if(!isLoggedIn)
        {
          if(nextProps.error !== 'Invalid email or password'){
            Actions.launch()
          }
