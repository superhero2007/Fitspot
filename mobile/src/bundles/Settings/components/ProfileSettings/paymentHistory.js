...
class PaymentHistory extends React.Component{
  ...
  constructor(props) {
    ...
    this.state = {
      items: props.workoutItems == null ? [] : props.workoutItems,
    ...
    return (
      <View style={{flex:1,alignSelf:'stretch'}}>
        <View style={{flexDirection:'row',marginTop:12,marginBottom:12,}}>
          <Text style={{fontFamily:'System',fontWeight:'500',color:'#4D4D4D',flex:1,marginLeft:16}}>{moment(rowData.date).format('DD MMM ha')}</Text>
          <Text style={{fontFamily:'System',fontWeight:'500',color:'#4D4D4D',flex:1,textAlign:'center',width:100}}>{rowData.user.firstName} {rowData.user.lastName}</Text>
          <Text style={{fontFamily:'System',fontWeight:'500',color:'#4D4D4D',flex:1,textAlign:'right',marginRight:16}}>${rowData.trainerPayout}</Text>
        </View>
        <HorizontalLine />
      </View>
    )
