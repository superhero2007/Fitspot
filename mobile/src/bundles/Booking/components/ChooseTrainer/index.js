...
class ChooseTrainer extends React.Component{
    ...
    constructor(props) {
      ...
      this.state = {
        ...
        isBrowsingTrainers: props.bookingState.bookingType == CONSTS.BOOKING_TYPE.BY_TRAINER,
        activitiesChosen: [],
        selectedTrainer: {},
        trainers: this.props.trainers,
        showActivityIndicator: props.showActivityIndicator,
        loadingText: 'Loading Trainers Near you.',
        currentItem: 0,
      }
    }
    ...
    convertTrainerArrayToMap(trainers){
      ...
      trainers.forEach(function(trainer){
        if(trainer.selectedTime || trainer.id < 0){
    ...
    filterTrainersByActivities(trainers){
      ...
      var newTrainers = trainers.filter(function(trainer){
    ...
    componentWillReceiveProps(nextProps){
      ...
      if(nextProps.trainers.length != 0 &&
        this.state.ds.length != nextProps.trainers.length){
          ...
          if(!this.state.isBrowsingTrainers){
            console.log('goin iiiin')
            console.log(nextProps.trainers)
            this.setState({
      ...
      if(nextProps.trainers.length == 0 && !nextProps.showActivityIndicator){

        this.setState({showActivityIndicator: nextProps.showActivityIndicator,
                        loadingText: 'No trainers available around this time and location.'})
      }
    ...
    selectTrainer(trainer){
      if(trainer.id < 0){
        ...
        for (var i = 0; i < this.state.trainers.length; i++) {
          if(this.props.trainers[i].id == trainer.id)
          {
            chosenIndex = i
          }
        }
        ...
        <TouchableHighlight onPress={() => this.selectTrainer(rowData)} style ={styles.rowStyle} underlayColor={'rgba(0,0,0,0)'}>
          <View style={{alignSelf:'center',justifyContent:'center',alignItems:'center', marginBottom:6}}>
            ...
            <View>
              ...
              { rowData.avatar ? <Image source={{uri: data.avatar.url}} style={{width:72,height:72 ,borderRadius:36}}/>
                : <Image source={require('@images/tab-profile.png')}
                    style={{width:72,height:72 ,borderRadius:36, backgroundColor: DEFAULT_GREEN_COLOR}}
                    resizeMode="center" />
              }
              {rowData.sessionInfo && rowData.sessionInfo.like ? <Icon name="star" size={15} color="#F6A623"
        ...
        <View style={styles.container}>
          ...
          <View style={{flex: 16}}>
              ...
              { this.state.ds.length > 0 ?
                ...
                :
                <LoadingDataView headerText={this.state.loadingText} descriptionText={''} showIndicator={this.state.showActivityIndicator} />
              }
          </View>

          <TrainerDetail currentItem={this.state.currentItem} visible={this.state.detailsVisible} selectedTrainer={this.state.selectedTrainer} trainers={ this.state.trainers }
