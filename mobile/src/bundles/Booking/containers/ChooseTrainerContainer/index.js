...
class ChooseTrainerContainer extends Component {
  ...
  refreshLocation(){
    ...
    navigator.geolocation.getCurrentPosition((position) => {
      ...
      if(bookingState.bookingType == CONSTS.BOOKING_TYPE.BY_ACTIVITY){
        ...
        ApiUtils.get(string).then(([response, jsonBody]) => {
          ...
          if (response.status == 200 && jsonBody.length > 0) {
            var newArray = [ ...this.state.trainers, ...jsonBody ];
            this.setState( {trainers: newArray});
          } else {
      ...
      Actions.mainAppModal(
      {
        ...
        detailsText: 'We\'ve encountered an issue with the geolocator. Please close the app and ensure you have allowed the app permissions for location.',
        onOkay:null,
        okayButtonText: 'OK',
        showCancelButton: false,
      }
  ...
  render() {
    return (<ChooseTrainer trainers={this.state.trainers} { ...this.props } showActivityIndicator={this.state.showActivityIndicator}/>)
  }
