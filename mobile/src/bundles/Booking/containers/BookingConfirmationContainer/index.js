...
class BookingConfirmationContainer extends Component {
  ...
  componentDidMount(){
    navigator.geolocation.getCurrentPosition((position) => {
      ...
      this.getNewPrice(0); // get 0 friends price.
    })
  }

  render() {
    return (
      <BookingConfirmation selectedPlan={this.props.selectedPlan}
        updatePayTitle={this.props.updatePayTitle}
        addBookingNonce={this.props.addBookingNonce}
        workoutInfo={this.props.workoutInfo} { ...this.state }
        beginEditingBooking={this.props.beginEditingBooking}
        updateNewPrice={(numberOfFriends) => this.getNewPrice(numberOfFriends)}
        requestNewWorkout={ (numberOfFriends) => this.requestNewWorkout(numberOfFriends)}  />
    )
  }
...
// workouts/estimate-price?lat=33.7806479&lon=-84.3861051&radius=50&dt=Wed%20Mar%2015%202017%2016%3A44%3A00%20GMT%2B0000&gym_id=1&numFriends=0
    ...
    ApiUtils.get(string).then(([response, jsonBody]) => {
      ...
      if (response.status == 200) {
        ...
        this.setState({bookingState: bookingState})
        var errorMessage = jsonBody.message
        if(errorMessage === 'App is not available'){
          errorMessage = 'We\'re sorry, but we cannot estimate the price of your workout because the app is not available in your area.';
        }
