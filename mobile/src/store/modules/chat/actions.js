    ...
    .then(([response, jsonBody]) => {
      if (response.status == 200) {
        ...
        var chatMessages = jsonBody.map(function(chatObj) {
          var url = chatObj.user.avatar === null ? null : chatObj.user.avatar.url;
...
export function sendReadMessages(sessionId) {
  ...
  return function(dispatch) {
    ...
    var string = 'chat/set-last-read-date/' + sessionId
    return ApiUtils.patch(string,{date: moment().utc().format() })
    .then(([response, jsonBody]) => {
      if (response.status == 200) {
