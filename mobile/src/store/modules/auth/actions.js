...
export const RESET_PASSWORD_START = `${NAMESPACE}/RESET_PASSWORD_START`;
export const RESET_PASSWORD_SUCCESS = `${NAMESPACE}/RESET_PASSWORD_SUCCESS`;
export const RESET_PASSWORD_FAIL = `${NAMESPACE}/RESET_PASSWORD_FAIL`;
...
function resetPasswordStart() {
  return {type: RESET_PASSWORD_START, receivedAt: Date.now()};
}

function resetPasswordSuccess(user) {
  return {type: RESET_PASSWORD_SUCCESS, user, receivedAt: Date.now()};
}

function resetPasswordFail(error) {
  return {type: RESET_PASSWORD_FAIL, error, receivedAt: Date.now()};
}
...
export function resetPassword(oldPassword, newPassword) {
  return function(dispatch) {
    ...
    ApiUtils.patch('auth/reset-password', {oldPassword: oldPassword, newPassword : newPassword})
      .then(([response, jsonBody]) => {
        if (response.status == 200) {
          dispatch(resetPasswordSuccess(jsonBody));
        } else {
          dispatch(resetPasswordFail(jsonBody.message));
        }
      })
      .catch(err => {
